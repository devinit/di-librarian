...
export const def = DefineFunction({
  ...
  input_parameters: {
    properties: {
      ...
      replyToReactionWorkflowCallbackId: { type: Schema.types.string },
    },
    required: ["interactivityPointer", "replyToReactionWorkflowCallbackId"],
  },
  output_parameters: {
    properties: {},
    required: [],
...
export default SlackFunction(def, async ({ inputs, client, env }) => {
  ...
  const triggerToUpdate = await findTriggerToUpdate(
    client,
    inputs.replyToReactionWorkflowCallbackId,
    debugMode,
  );
  if (debugMode) {
  ...
  .addViewSubmissionHandler(
    ...
    async ({ view, inputs, client, env }) => {
      const debugMode = isDebugMode(env);
      const { replyToReactionWorkflowCallbackId } = inputs;
      const conversationIds =
        view.state.values.block.channels.selected_conversations;

      let modalMessage =
        "*You're all set!*\n\nThe librarian is now available for the channels :white_check_mark:";
      try {
    ...
    "blocks": [
      {
        ...
        "label": {
          "type": "plain_text",
          "text": "Channels to enable librarian",
        },
      },
    ],
  };
